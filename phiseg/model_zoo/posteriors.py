...
def betaVAE_bn(x, s_oh, zdim_0, training, scope_reuse=False, norm=tfnorm.batch_norm, **kwargs):
    ...
    with tf.variable_scope('posterior') as scope:
        ...
        for ii in range(resolution_levels-1):
            net = layers.conv2D(net, 'q_z_%d' % ii, num_filters=n0*(ii//2+1), kernel_size=(4,4), strides=(2,2), normalisation=norm, training=training)

        net = layers.conv2D(net, 'q_z_%d' % resolution_levels, num_filters=n0*8, kernel_size=final_kernel_size, strides=(1,1), padding='VALID', normalisation=norm, training=training)
...
def unet_T_L(x, s_oh, zdim_0, training, scope_reuse=False, norm=tfnorm.batch_norm, **kwargs):
...
def segvae_const_latent(x, s_oh, zdim_0, training, scope_reuse=False, norm=tfnorm.batch_norm, **kwargs):
...
def hybrid(x, s_oh, zdim_0, training, scope_reuse=False, norm=tfnorm.batch_norm, **kwargs):
